function m2lyx(M, file_name, header, format)
...
function str = format_cell(x, format)
    str = '';
    if iscell(x)
        x = x{1};
    end
    ...
    elseif isfloat(x) && length(x) == 1
        str = [str, sprintf([format '\n'], x)];
    elseif isfloat(x) && length(x) > 1
        str = [str, sprintf('%s\n', ...
